...
// Add Zen Tabs styles
if (theme_get_setting('corp_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'corp') .'/css/tabs.css');
}

function corp_preprocess_html(&$vars) {
  global $user;

  //Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . corp_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = corp_id_safe('page-'. $path);
    $vars['classes_array'][] = corp_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('corp_ie_enabled')) {
    $corp_ie_enabled_versions = theme_get_setting('corp_ie_enabled_versions');
    if (in_array('ie8', $corp_ie_enabled_versions, TRUE)) {
...
function corp_preprocess_page(&$vars, $hook) {
...
function corp_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from corp_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function corp_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function corp_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('corp_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('corp_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('corp_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('corp_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('corp_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function corp_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = corp_id_safe($element['#title']);
 ...
 */
function corp_preprocess_menu_local_task(&$variables) {
 ...
 */
function corp_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function corp_process_html(&$variables) {
 ...
 */
function corp_process_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
 ...
 */
function corp_build_columns($columns) {
  $styles = array();
  $num_columns = 0;
  $first = -1;
